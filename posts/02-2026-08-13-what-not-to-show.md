...
터미널에서 Claude Code 대화를 여러 개 띄워 작업하다 보니 새 문제가 생겼다. **지금 어디까지 와 있는지가 안 보인다.**
...
세 칸이다. 위에는 내가 확인해야 할 일, 가운데는 다시 열어볼 링크와 초안을 고정해두는 ‘핀’, 아래는 손댄 파일. 10초마다 갱신되고 터미널 아래쪽 몇 줄만 차지한다.
...
적어둔 게 없어졌다는 뜻으로 읽힌다. 처음엔 그렇게 읽고 패널 버그를 의심했다. 몇 번 겪고 나서야 알았는데, **목록이 안 보인다고 해서 데이터가 사라진 것은 아니었다.**
...
**둘. 비어 있는 게 정답일 때도 있다.** 패널 전체가 아니라 할 일 칸만 비는 경우도 있었다. 할 일을 전부 끝냈다면 자연스러운 결과다. 어디가 비었는지와 실제로 남은 일이 있는지를 함께 봐야 했다.

그래서 화면을 의심하기 전에 **패널이 지금 세션의 목록을 보고 있는지**부터 확인한다. 연결이 맞다면 저장된 목록과 화면을 비교한다. 목록에 항목이 있는데 안 보이면 표시 문제이고, 목록도 비어 있다면 완료해서 비운 것인지 확인한다.
...
**3차** — 당시에는 기준을 좁혔다. **"나를 위해 만들어진 것만."** 작업하느라 거쳐 간 파일은 빼고, 글이나 초안처럼 결과로 남은 것만 보고 싶었다.
...
이유는 내가 쓰던 도구가 파일을 저장하는 방식에 있었다. 원본을 직접 고치는 대신 임시 파일을 새로 만들어 내용을 다 쓴 다음, 그걸 원래 이름으로 바꿔치기한다. 쓰다가 죽어도 반쯤 망가진 파일이 안 남게 하려는 것이다.
...
쓰다 보니 기준도 다시 넓힐 필요가 있었다. 새 결과물뿐 아니라 **지금 수정 중인 코드도 바로 열어보고 싶었다.** 맨 위 예시에 소스와 테스트 파일이 있는 이유다. 그런데 셸로 내용을 직접 고치는 명령은 수집 대상에서 빠져 있어, 그런 수정은 놓쳤다.

**결국 내가 다시 열어볼 파일인지까지는 자동으로 판별하지 못했다.** 결과물과 작업 파일을 놓치지 않도록 조금 더 담는 쪽을 택했다.
...
내가 결정할 일을 따로 적기로 하니, 기록하는 습관도 필요해졌다. 그래도 이 칸은 남겨뒀다. **잊어서 비는 건 습관으로 고칠 수 있고, 봐도 눈에 안 들어오는 건 고치기 어렵다.**
