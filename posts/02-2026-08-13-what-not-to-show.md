...
그래서 지금은 진단 순서를 정해뒀다. 먼저 적어둔 목록이 실제로 있는지 본다. 목록에 있는데 화면에 없으면 그리는 쪽 문제고, 목록까지 비어 있으면 그때 식별자를 의심한다.

**"안 보인다"에서 곧장 "없어졌다"로 가면 안 된다**는 걸 세 번 만에 배웠다.

그리고 이 절을 다 쓴 뒤에 **네 번째가 나왔다.** 핀을 꽂는 명령의 기본 동작이 "목록에 추가"가 아니라 **"목록을 통째로 교체"** 였다. 여러 개를 연달아 꽂으면 마지막 하나만 남는다. 에러도 없고, 마지막 항목은 멀쩡히 떠 있으니 나머지가 사라졌다는 걸 한참 뒤에야 안다. 추가하는 줄 알았던 명령이 지우고 있었다.

기본값을 "추가"로 뒤집고, 전체 교체는 따로 명시해야만 되게 바꿨다. **실수했을 때 잃는 쪽이 아니라 쌓이는 쪽이 기본값이어야 한다.**
...
며칠 쓰다가 가운데 칸이 눈에 걸렸다. 이번 세션의 할 일을 보여주던 칸이다. 처음엔 이게 제일 쓸모 있다고 생각했는데, 보다 보니 어딘가 익숙했다.

**에이전트가 이미 화면 위쪽에 같은 걸 그리고 있었다.**

할 일 목록은 원래 대화창에 표시된다. 패널은 그걸 아래쪽에 한 번 더 그리고 있었을 뿐이다. 두 곳에 같은 정보가 있으니 어느 쪽을 봐야 할지 매번 잠깐 헷갈렸고, 그 잠깐이 쌓였다.

그래서 뺐다. 몇 줄짜리 화면에서 세 칸 중 하나를 통째로.

빼고 나니 **이름이 안 맞았다.** "할 일 패널"이라고 불러왔는데 남은 건 링크와 파일이다. 그래서 이름도 바꿨다. 할 일이 아니라 맥락을 담는 판이 됐으니까.

### 그런데 며칠 만에 비슷한 게 다시 생겼다

링크를 꽂는 칸에 나도 모르게 **할 일을 적고 있었다.** "이건 나중에 확인", "이 판단은 보류" 같은 것들이다. 링크와 메모가 한 칸에 섞이니 다시 구분이 안 됐다.

그래서 그 칸에서 할 일만 골라 별도 칸으로 뺐다. 결국 다시 세 칸이 됐다.
...
도구 이름은 바꾼 채로 뒀다. 다시 들어온 할 일도 결국 **내가 적어 넣은 맥락**이라, 처음의 "할 일 패널"보다는 지금 이름이 맞다.

### 그리고 손으로 채우는 걸 잊었다

며칠 뒤 패널을 보니 할 일 칸이 없었다. 남은 일이 없어서가 아니라 **적어 넣는 걸 잊어서**였다.

자동으로 채워지던 때는 신경 쓸 일이 아니었다. 일을 시작하면 목록이 알아서 생겼으니까. 손으로 적기로 바꾸고 나서는 **적는 행위가 하나 늘었는데**, 그 습관이 아직 안 붙은 것이다.

빼는 데에도 비용이 있다. 자동으로 되던 걸 끄면 그 자리는 사람이 메워야 하고, 사람은 잊는다. 중복을 지운 대가로 **빈칸이 생길 위험**을 받아온 셈이다.

그래도 되돌리지는 않았다. 자동으로 채워진 목록은 잊지 않고 뜨지만 내가 안 봤고, 손으로 적은 목록은 가끔 비지만 적어둔 것만은 정확히 내가 남긴 것이다. 잊어서 비는 건 습관으로 고칠 수 있고, 봐도 눈에 안 들어오는 건 고치기 어렵다.

---

## 결국, 빈 화면을 택했다

패널은 자기가 어느 세션에 붙어 있는지를 탭 기록으로 안다. 그런데 그 기록이 없을 때가 있다. 처음엔 이럴 때 **가장 최근에 움직인 세션**을 대신 보여주게 해뒀다. 뭐라도 띄우는 게 빈 화면보다 낫다고 생각했다.

아니었다.

옆 세션의 링크가 내 세션 것처럼 떠 있으면, 나는 그게 남의 것인 줄 모른다. 화면은 멀쩡하고 항목도 그럴듯하다. **틀렸다는 신호가 어디에도 없다.** 빈 화면은 최소한 "여기 아무것도 없다"는 사실만은 정확히 말해주는데, 남의 것으로 채워진 화면은 거짓을 정확한 얼굴로 말한다.

그래서 폴백을 지웠다. 지금은 세션을 특정하지 못하면 이렇게만 뜬다.

```
CONTEXT  세션 미상
이 탭이 어느 세션 것인지 알 수 없어 아무것도 보여주지 않습니다.
```

**틀릴 바에는 비워두는 쪽으로 정한 것이다.** 이 글에서 지금까지 뺀 것들은 전부 "필요 없어서" 뺐는데, 이건 다르다. 있으면 도움이 될 수도 있는 정보를 **틀릴 가능성 때문에** 뺐다.
...
그리고 뺀 이유는 두 갈래였다. 대부분은 **없어도 되는 것**이라 뺐고, 마지막 하나는 **틀릴 수 있는 것**이라 뺐다. 앞의 것은 화면을 정리하지만, 뒤의 것은 화면을 믿을 수 있게 만든다.
