...
처음 만들었을 때도 칸은 셋이었다. 다만 가운데가 지금과 달랐다 — 이번 세션의 할 일을 에이전트가 자동으로 채워 넣는 칸이었다. 처음엔 이게 제일 쓸모 있다고 생각했는데, 며칠 쓰다 보니 어딘가 익숙했다.
...
그래서 그 칸에서 할 일만 골라 별도 칸으로 뺐다. 맨 위에 보여준 화면이 그 결과다.
